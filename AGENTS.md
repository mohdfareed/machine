...
- `app/cli/` - CLI entrypoint, separate deploy/update/sync commands, info commands, and shared reporting
...
- `mc sync` fetches canonical `mohdfareed/machine` main, integrates with Git's
  fast-forward-only merge with autostash, then deploys the current checkout.
  Tracked local edits are restored before deployment; ahead commits and detached HEAD
  are allowed. Git failures or autostash restoration conflicts stop before deployment.
...
- `mc deploy` only installs missing packages; upgrades belong to `mc update`. If a package exists but is not managed by the requested manager, `mc deploy` should still install it with that manager
- Script-only packages use `Package.name` as the installed-command check during `mc deploy`; `mc update` re-runs script-backed packages
...
- `~/.env` is the only generated file - written by `mc deploy` with just `MC_HOME` and `MC_ID`
- Three-tier script environment (`app.env.build_env`):

  1. `~/.env` - generated by `mc deploy`, contains only `MC_HOME` and `MC_ID`
...
- State file: `app_dir/state.json` tracks script runs; package presence is determined from the requested package manager at deployment time
...
- App data: `typer.get_app_dir(Settings.command)` for logs/state; derive the command from package entry-point metadata, define runtime file paths once in `Settings`, and reuse them in readers, writers, and CLI commands
