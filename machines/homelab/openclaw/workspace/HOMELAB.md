...
| **Qdrant**               | `qdrant/qdrant`                                     | Tailnet on `mem0.<tailnet>.ts.net:6333` via the OpenMemory sidecar  | Shared vector store; currently used by OpenMemory/mem0 |
...
- `openclaw-mem0` - **active memory provider** (OSS mode, using the shared Qdrant backend on `mem0.<tailnet>.ts.net:6333`)
